...
@[simp] lemma Subgraph.edgeSet_map {G : SimpleGraph α} {H : SimpleGraph β} (f : G →g H) (G' : G.Subgraph) :
...
@[simps!]
noncomputable def Subgraph.isoMap {H : SimpleGraph β} (f : G →g H) (hf : Injective f) (G' : G.Subgraph) :
    G'.coe ≃g (G'.map f).coe :=
...
@[simps]
def Subgraph.topIso : (⊤ : G.Subgraph).coe ≃g G where
  toFun := (↑)
...
@[simp] lemma Subgraph.coe_bot : (⊥ : G.Subgraph).coe = ⊥ := rfl

noncomputable instance {V : Type*} {G : SimpleGraph V} [Fintype V] : Fintype G.Subgraph :=
  Fintype.ofEquiv
...
@[simp] lemma Subgraph.edgeSet_coe {G' : G.Subgraph} :
    G'.coe.edgeSet = Sym2.map (↑) ⁻¹' G'.edgeSet := by
  ext e; induction' e using Sym2.ind with a b; simp

lemma Subgraph.image_coe_edgeSet_coe (G' : G.Subgraph) : Sym2.map (↑) '' G'.coe.edgeSet = G'.edgeSet := by
...
lemma isContained_iff_exists_subgraph :
    (∃ f : G →g H, Injective f) ↔ ∃ H' : H.Subgraph, Nonempty (G ≃g H'.coe) := by
  constructor
  · rintro ⟨f, hf⟩
    exact ⟨Subgraph.map f ⊤, ⟨(Subgraph.isoMap _ hf _).comp Subgraph.topIso.symm⟩⟩
  · rintro ⟨H', ⟨e⟩⟩
    exact ⟨H'.hom.comp e.toHom, Subtype.val_injective.comp e.injective⟩
...
@[simp] lemma copyCount_pos : 0 < G.copyCount H ↔ ∃ f : G →g H, Injective f := by
  simp [copyCount, isContained_iff_exists_subgraph, card_pos, filter_nonempty_iff]

@[simp] lemma copyCount_eq_zero : G.copyCount H = 0 ↔ ∀ f : G →g H, ¬Injective f := by
  simp [←not_iff_not, ←ne_eq, ←pos_iff_ne_zero, copyCount_pos]
...
noncomputable def labelledCopyCount (G : SimpleGraph α) (H : SimpleGraph β) : ℕ :=
  Fintype.card {f : G →g H // Injective f}
...
@[simp] lemma labelledCopyCount_eq_zero :
    G.labelledCopyCount H = 0 ↔ ∀ f : G →g H, ¬Injective f := by
  simp [labelledCopyCount, Fintype.card_eq_zero_iff]

@[simp] lemma labelledCopyCount_pos :
    0 < G.labelledCopyCount H ↔ ∃ f : G →g H, Injective f := by
  simp [labelledCopyCount, Fintype.card_pos_iff]
...
lemma kill_eq_right (hG : G ≠ ⊥) : G.kill H = H ↔ ¬ ∃ f : G →g H, Injective f := by
...
lemma kill_of_not_isContained (hGH : ¬ ∃ f : G →g H, Injective f) : G.kill H = H := by
...
contain `G`. -/
lemma not_isContained_kill (hG : G ≠ ⊥) : ¬ ∃ f : G →g G.kill H, Injective f := by
